...
Menu rendering process:
    ...
    -   create set of choices (apply filters in context, permissions,
        condition)
...
<body>
    ...
    <ul tal:repeat="item menu/main">
        <a tal:attributes="href item/action" tal:content="item/title"></ul>
    </ul>
...
main = menu.Menu(
    # Link to a view. Necessary information: context, name
    menu.View(EditContact),
    # Link to a generic action/URL
    menu.Action("External action", "string:http://${context/"),
    menu.SubMenu(
        menu.Item()))
